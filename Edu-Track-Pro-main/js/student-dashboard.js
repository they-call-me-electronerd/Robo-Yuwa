...
document.addEventListener('DOMContentLoaded', () => {
    // Check session - allow admin bypass
    const isAdminBypass = sessionStorage.getItem('adminBypass') === 'true';
    if (!isAdminBypass && (sessionStorage.getItem('isLoggedIn') !== 'true' || sessionStorage.getItem('userRole') !== 'student')) {
        window.location.href = 'login.html';
    }
    
    // Show "Back to Admin" button if accessed via admin
    if (isAdminBypass) {
        const backToAdminLink = document.getElementById('backToAdminLink');
        if (backToAdminLink) {
            backToAdminLink.style.display = 'block';
        }
    }
